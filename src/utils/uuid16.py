...
class UUID16:
    ...
    def __init__(
        ...
        if time_low is not None:
            ...
            fields[0] = time_low
            self._uuid = UUID(fields=fields)  # type: ignore
        else:
            if UUID16.is_in_range(uuid):  # type: ignore
                self._uuid = uuid  # type: ignore
            else:
    ...
    @classmethod
    def parse_uuid(cls, uuid: Union[str, bytes, int, UUID]) -> Union[UUID, "UUID16"]:  # type: ignore
